...
export default function AccountOrdersPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full font-mono text-ink">
      ...
      <div className="border-b-4 border-ink pb-4 mb-8 flex items-center justify-between">
        ...
        {user && (
          <button
            ...
            className="text-xs font-bold uppercase text-signal hover:underline border border-ink px-3 py-1.5 bg-paper"
          >
            SIGN OUT
          </button>
        )}
      ...
      {!user ? (
        /* Sign-In Form for Guest / Existing Users */
        <div className="bg-paper border-std p-6 sm:p-10 space-y-6 max-w-lg mx-auto">
          ...
          {authMessage && (
            <div className="bg-tape/20 border-2 border-ink p-3 text-xs font-bold text-ink">
              📢 {authMessage}
            </div>
          )}
          ...
          <form onSubmit={handleMagicLinkLogin} className="space-y-4 text-xs">
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full bg-paper border-2 border-ink p-3 text-xs focus:outline-hidden"
              />
            </div>

            <button
              type="submit"
              className="w-full bg-signal text-paper py-3.5 px-4 font-mono font-bold text-xs sm:text-sm uppercase border-2 border-ink shadow-md hover:bg-signal/90 transition-colors active:translate-y-0.5"
            >
              SEND MAGIC LINK
            </button>
          </form>

          <div className="pt-4 border-t border-static text-[11px] text-ink/60 flex items-center gap-2">
        ...
        <div className="space-y-6">
          <div className="bg-paper border-std p-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-ink text-paper flex items-center justify-center font-bold">
                <User className="w-5 h-5" />
              </div>
              <div>
                <div className="text-xs font-bold text-ink/60 uppercase">SIGNED IN AS</div>
          ...
          <div className="bg-paper border-std p-6 space-y-4">
            <h2 className="text-sm font-bold uppercase text-ink flex items-center gap-2 border-b border-static pb-2">
              <Package className="w-4 h-4 text-signal" />
              PAST ORDERS ARCHIVE
            </h2>

            {/* Sample order item representation */}
            <div className="border-2 border-ink p-4 space-y-3 bg-paper">
              <div className="flex items-center justify-between text-xs font-bold">
                <span className="text-ink uppercase">ORDER #ORD-1998-DD</span>
                <span className="bg-crt/20 text-crt px-2 py-0.5 border border-crt text-[10px]">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-static text-xs">
                <span className="text-ink/60">TOTAL: {formatPaise(185000)}</span>
                <span className="text-tape font-bold">TRACKING: BLUEDART-IND-88219</span>
              </div>
            </div>
          </div>
        </div>
