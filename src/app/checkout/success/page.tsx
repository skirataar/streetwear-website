...
function SuccessContent() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12 font-mono text-ink">
      {/* STD Booth Paper Receipt Box — white card on hype green */}
      <div className="bg-white border-std p-6 sm:p-10 space-y-6 shadow-xl relative">
        {/* Receipt Header */}
        <div className="text-center border-b-2 border-dashed border-ink pb-6 space-y-2">
          <div className="inline-flex items-center justify-center w-12 h-12 bg-hype border-2 border-ink rounded-full mb-1">
            <CheckCircle2 className="w-7 h-7 text-ink" />
          </div>
          <div className="text-xs font-bold uppercase text-flash tracking-widest">
        ...
        <div className="grid grid-cols-2 gap-4 text-xs border-b-2 border-dashed border-ink pb-6">
          <div>
            <span className="text-ink/50 uppercase block text-[10px]">ORDER DOCKET NUMBER</span>
            <span className="font-bold text-sm text-ink">{orderId}</span>
          </div>
          <div className="text-right">
            <span className="text-ink/50 uppercase block text-[10px]">PAYMENT STATUS</span>
            <span className="font-bold text-sm text-hype bg-ink px-2 py-0.5 inline-block">
              PAID // RAZORPAY
            </span>
          </div>
          <div>
            <span className="text-ink/50 uppercase block text-[10px]">DISPATCH TIMELINE</span>
            <span className="font-bold text-xs text-ink">24-48 HOURS (EXPRESS)</span>
          </div>
          <div className="text-right">
            <span className="text-ink/50 uppercase block text-[10px]">TIMESTAMP</span>
            <span className="font-bold text-xs text-ink">{currentDate || "LIVE"}</span>
          </div>
        </div>

        {/* Liner Note Message */}
        <div className="bg-hype/20 border-2 border-ink p-4 space-y-2 text-xs">
          <div className="font-bold uppercase text-flash flex items-center gap-1.5">
            <span>● DISPATCH INSTRUCTIONS</span>
          </div>
          <p className="font-body text-ink/70 text-xs leading-relaxed">
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t-2 border-dashed border-ink">
          <button
            ...
            className="flex-1 inline-flex items-center justify-center gap-2 bg-white text-ink border-2 border-ink py-3 px-4 text-xs font-bold uppercase hover:bg-ink/5 transition-colors"
          >
        ...
        {/* Bottom Bar */}
        <div className="pt-2 text-center text-[10px] text-ink/40 font-bold uppercase">
...
export default function OrderSuccessPage() {
  return (
    <Suspense fallback={<div className="p-12 text-center font-mono text-ink">LOADING TRANSMISSION...</div>}>
      <SuccessContent />
    </Suspense>
  );
}
